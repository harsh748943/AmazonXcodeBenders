...
public class OpenAIHelper {
   ...
   // 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
    public static void extractBillInfo(String smsText, BillCallback callback) {
        ExecutorService executor = Executors.newSingleThreadExecutor();
        Handler handler = new Handler(Looper.getMainLooper());
        ...
        executor.execute(() -> {
            try {
                ...
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("POST");
                conn.setRequestProperty("Authorization", "Bearer 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");
                conn.setRequestProperty("Content-Type", "application/json");
                conn.setDoOutput(true);
...
//                message.put("content", "Extract bill info from this message: '" + smsText + "'. Reply only in JSON with keys: amount, due_date, description.");
                ...
                message.put("content", "Extract bill info from this SMS: '" + smsText + "'. Respond ONLY in pure JSON with keys: amount, due_date, bill_type, and description. Example: {\"amount\":\"₹1200\", \"due_date\":\"25 June\", \"bill_type\":\"Electricity Bill\", \"description\":\"Your electricity bill is due.\"}");
